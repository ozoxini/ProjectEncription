...
class AESCore:
    ...
    def _mix_columns(self, state: list[int]) -> list[int]:
        """Operacja MixColumns: Mnożenie przez stałą macierz w ciele Galois."""
        new_state = [0] * 16
        for c in range(4):
            i = c * 4
            s0 = state[i]
    ...
    def encrypt_block(self, blok_jawny: bytes, expanded_key: list[list[int]]) -> bytes:
    ...
    def decrypt_block(self, blok_szyfrogramu: bytes, expanded_key: list[list[int]]) -> bytes:
